...
function ExecutePageContent() {
    ...
    // Updated handleExecute function for your execute page
    // Updated handleExecute function for your execute page
    const handleExecute = async () => {
        ...
        try {
            ...
            try {
                // The transaction data from your backend is likely a base64 string in the "bytes" field
                if (transactionData && typeof transactionData === 'object' && 'bytes' in transactionData) {
                    // Your backend is sending { bytes: "base64string" }
                    const bytes = transactionData.bytes;
                    if (typeof bytes === 'string') {
                        console.log('Using bytes field for transaction');
                        tx = Transaction.from(bytes);
                    } else {
                        throw new Error('Bytes field is not a string');
                    }
                }
                // Handle if it's just a string directly
                else if (typeof transactionData === 'string') {
                    console.log('Using string data for transaction');
                    tx = Transaction.from(transactionData);
                }
                // Handle if it's raw bytes
                else if (transactionData instanceof Uint8Array) {
                    console.log('Using Uint8Array for transaction');
                    tx = Transaction.from(transactionData);
                }
                else {
                    console.error('Unsupported transaction data format:', transactionData);
                    throw new Error('Unsupported transaction data format. Expected base64 string or bytes.');
                }

                console.log('Transaction successfully parsed:', tx);
            ...
            // Check transaction success more thoroughly
            if (result.effects?.status?.status === 'success' || result.digest) {
                ...
                const errorMsg = result.effects?.status?.error || 'Transaction status unclear';
                setStatus(`Transaction may have failed: ${errorMsg}`);
                console.error('Transaction status unclear:', result);
            }
